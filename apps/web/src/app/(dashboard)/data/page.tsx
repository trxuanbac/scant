...
function DatasetPreview({ profile, activeSheet, onSelectSheet }: { profile: DatasetProfile; activeSheet: number; onSelectSheet: (index: number) => void }) {
  ...
  return <div className="space-y-4 text-xs sm:text-sm">
    ...
    {facts.length > 0 && <section className="overflow-hidden rounded-lg border border-indigo-100"><h3 className="flex items-center gap-2 border-b border-indigo-100 bg-indigo-50 px-3 py-2.5 font-semibold text-indigo-950"><BarChart2 className="h-4 w-4" />Số liệu đã kiểm chứng</h3><div className="max-h-64 divide-y divide-slate-100 overflow-y-auto lg:max-h-72">{facts.slice(0, 18).map((fact) => <div key={fact.id} className="grid grid-cols-[72px_minmax(0,1fr)] gap-3 px-3 py-2.5 lg:grid-cols-[84px_minmax(280px,1.2fr)_minmax(240px,1fr)]"><span className="font-mono font-medium text-indigo-700">{fact.id}</span><span className="min-w-0 font-medium text-slate-800">{fact.fact}</span><span className="col-start-2 min-w-0 break-words text-slate-500 lg:col-start-auto">{typeof fact.value === "object" ? JSON.stringify(fact.value) : String(fact.value ?? "—")}</span></div>)}</div></section>}
    <section className="overflow-hidden rounded-lg border border-slate-200"><h3 className="border-b border-slate-100 px-3 py-2.5 font-semibold text-slate-900">Cột dữ liệu{sheet.name ? ` · ${sheet.name}` : ""}</h3><div className="max-h-52 divide-y divide-slate-100 overflow-y-auto lg:max-h-64">{columns.length ? columns.map((column) => <div key={column.name} className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 px-3 py-2.5 sm:grid-cols-[minmax(0,1fr)_120px_120px]"><span className="truncate font-medium text-slate-800">{column.name}</span><span className="text-slate-500">{column.type || "—"}</span><span className="col-start-1 text-slate-400 sm:col-start-auto">{column.unique_count ?? "—"} giá trị</span></div>) : <p className="px-3 py-4 text-slate-500">Chưa có thông tin cột.</p>}</div></section>
